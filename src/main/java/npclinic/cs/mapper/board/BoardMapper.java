...
public interface BoardMapper {
    ...
    @Update("UPDATE board SET views=#{views} WHERE id=#{id}")
    void updateViews(int id, int views);

    @Update("UPDATE board SET title=#{title}, content=#{content}, views=#{views}+1 WHERE id=#{id}")
